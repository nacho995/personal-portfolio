...
import { FaGithub, FaExternalLinkAlt, FaCode, FaLaptopCode, FaMobileAlt, FaServer, FaDatabase, FaRocket } from 'react-icons/fa';

const CATEGORY_CONFIG = {
  'tenfe': { label: 'Sistema de Reservas', icon: FaServer, gradient: 'from-emerald-500 to-teal-600' },
  'goza-madrid': { label: 'Real Estate', icon: FaLaptopCode, gradient: 'from-orange-500 to-red-600' },
  'codlet': { label: 'Desarrollo Web', icon: FaCode, gradient: 'from-blue-500 to-indigo-600' },
  'hacienda-san-carlos': { label: 'Landing Page', icon: FaMobileAlt, gradient: 'from-amber-500 to-orange-600' },
  'mundo-tinta': { label: 'E-Commerce', icon: FaRocket, gradient: 'from-pink-500 to-rose-600' },
  'biologic-analysis': { label: 'Data Science', icon: FaDatabase, gradient: 'from-purple-500 to-indigo-600' },
  'portfolio-gilberto': { label: 'Portfolio', icon: FaLaptopCode, gradient: 'from-cyan-500 to-blue-600' },
};

const DEFAULT_CATEGORY = { label: 'Web Project', icon: FaCode, gradient: 'from-violet-500 to-purple-600' };
...
const GenericProject = ({
  ...
  const category = CATEGORY_CONFIG[project.id] || DEFAULT_CATEGORY;
  const CategoryIcon = category.icon;
  const techStack = project.techStack || [];
  const slugPath = project.id || 'project';
  ...
  return (
    ...
    >
      ...
      {/* Border glow animado */}
      <div
        className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 animate-neon-pulse"
        style={{ boxShadow: `inset 0 0 30px ${accentColor}30` }}
      />

      {/* Imagen del proyecto con overlay */}
      <div className="relative aspect-video overflow-hidden bg-gradient-to-br from-gray-950 via-slate-900 to-gray-950">
        <motion.img
          src={project.image}
        ...
        <div className="absolute top-4 right-4">
          <span className={`px-4 py-2 bg-gradient-to-r ${category.gradient} text-white text-sm font-bold rounded-full shadow-lg backdrop-blur-sm flex items-center gap-2`}>
            <CategoryIcon />
            {category.label}
          </span>
        </div>
      ...
      <div className="relative p-6 sm:p-8">
        ...
        <div className="flex items-center gap-2 mb-4 pb-2 border-b" style={{ borderColor: `${accentColor}30` }}>
          <span className="font-code text-xs opacity-60" style={{ color: accentColor }}>
            ~/projects/{slugPath}/
          </span>
          <span
            className="font-code text-xs px-2 py-0.5 rounded border"
        ...
        <div className="flex justify-between items-start mb-4">
          <div className="flex-1">
            <h3
              className="text-2xl sm:text-3xl font-bold font-code mb-2 transition-all duration-300"
              style={{
                color: 'white',
          ...
          <div className="flex flex-wrap gap-2">
            {techStack.map((tech, index) => (
              <motion.span
                ...
                style={{
                  backgroundImage: `linear-gradient(135deg, ${tech.color.replace('from-', '').replace('to-', '').split(' ')[0]}15, transparent)`,
...
GenericProject.propTypes = {
  project: PropTypes.shape({
    ...
    githubBackendUrl: PropTypes.string,
