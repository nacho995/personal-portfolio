...
import { FaGithub, FaExternalLinkAlt, FaMicroscope, FaCube } from 'react-icons/fa';

const BiologicProject = ({
  project,
  tempRating,
  rating,
  averageRating,
  ...
  const techStack = [
    ...
    { name: 'TIFF Processing', color: '#6B46C1' }
  ];
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-video overflow-hidden bg-gradient-to-br from-purple-950 via-indigo-900 to-blue-950">
        {/* Imagen real */}
        <motion.img 
          src={project.image}
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-2">
          <span className="px-4 py-2 bg-gradient-to-r from-purple-500 to-indigo-600 text-white text-sm font-bold rounded-full shadow-lg backdrop-blur-sm flex items-center gap-2">
            <FaMicroscope />
            {t('biologic.badge')}
          </span>
        </div>
        ...
        <div className="absolute top-4 left-4">
          <span className="px-3 py-1.5 bg-gradient-to-r from-cyan-500 to-blue-500 text-white text-xs font-bold rounded-full shadow-lg backdrop-blur-sm flex items-center gap-2">
            <FaCube />
            3D Volumetric
          </span>
        </div>
      ...
      <div className="relative p-6 sm:p-8">
        ...
        <div className="flex items-center gap-2 mb-4 pb-2 border-b" style={{ borderColor: `${accentColor}30` }}>
          <span className="font-code text-xs opacity-60" style={{ color: accentColor }}>~/projects/biologic-analysis/</span>
      ...
      {/* Efecto de partículas en hover - Temática científica */}
      <div className="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-500">
        {[...Array(8)].map((_, i) => (
          <motion.div
            key={i}
            className="absolute text-purple-500/40 text-3xl"
            style={{
...
export default BiologicProject;
